...
*One... is fine*
The quickstart relies on that everything needed to connect to Nats is provided by you. That entails a NATS connection and a codec (`codec.Json()`). With that set you can go straight for the desert by using the default RPC instance with `rpc.HandleRPC` and `rpc.HandleEvent`.
...
* context tests needs to be redone and focuesed on the contexts.